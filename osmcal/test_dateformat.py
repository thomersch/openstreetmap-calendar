...
class DateFormatTest(SimpleTestCase):
    ...
    def test_dateformat_start(self):
        self.assertEqual(
            self._fmt(MockEvent(start=datetime(year=self.cur.year, month=1, day=1), whole_day=True)), "1st January"
        )

    def test_dateformat_start_with_time(self):
        self.assertEqual(
            self._fmt(MockEvent(start=datetime(year=self.cur.year, month=1, day=1, hour=18))), "1st January 18:00"
        )
    ...
    def test_dateformat_start_end_with_time(self):
        self.assertEqual(
            self._fmt(
                MockEvent(
                    start=datetime(year=self.cur.year, month=1, day=2, hour=16),
                    end=datetime(year=self.cur.year, month=1, day=2, hour=18),
                )
            ),
    ...
    def test_dateformat_start_end_with_time_multiday(self):
        self.assertEqual(
            self._fmt(
                MockEvent(
                    start=datetime(year=self.cur.year, month=1, day=2, hour=16),
                    end=datetime(year=self.cur.year, month=1, day=3, hour=18),
                )
            ),
    ...
    def test_localized_de(self):
        ...
        self.assertEqual(
            self._fmt_loca(MockEvent(start=datetime(year=self.cur.year, month=10, day=3), whole_day=True)), "3. Oktober"
        )
    ...
    def test_localized_fr(self):
        ...
        self.assertEqual(
            self._fmt_loca(MockEvent(start=datetime(year=self.cur.year, month=10, day=3), whole_day=True)), "3 Octobre"
        )
        translation.deactivate()
