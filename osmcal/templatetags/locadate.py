...
from babel.dates import format_interval, format_skeleton, get_timezone, get_timezone_name
...
def loca_day_month_fmt(lang):
    ...
    if fmt_str == "PLACEHOLDER":
        ...
        jS F would be 1st May in en, but in everything else we'd have 1st Май, which doesn't make any sense, so we have
        a placeholder in the translation and going to fallback to j E which is reasonable in most languages.
        """
...
def with_timezone(original, evt, locale, user_tz: tzinfo = None):
    ...
    return original + " " + get_timezone_name(dt.tzinfo, width="short", locale=locale)


@register.simple_tag()
def full_date_format(evt, locale=DEFAULT_LOCALE, user_tz: tzinfo = None) -> str:
    """
    ...
    if evt.end:
        if evt.whole_day:
            return format_interval(evt.start.date(), evt.end.date(), "yMMMMd", tzinfo=tz, locale=lo)
        else:
            if evt.start.date() == evt.end.date():
                fmt = lo.datetime_formats["medium"].replace("'", "")
                return with_timezone(
                    fmt.format(
                        format_interval(evt.start.time(), evt.end.time(), "Hm", tzinfo=tz, locale=lo),
            ...
            return with_timezone(
                format_interval(evt.start, evt.end, skeleton="yMMMMd Hm", tzinfo=tz, locale=lo), evt, lo, user_tz
            )
...
@register.simple_tag()
def short_date_format(evt, locale=DEFAULT_LOCALE, user_tz: tzinfo = None) -> str:
    lo = Locale.parse(locale, sep="_")
    tz = get_timezone(evt.timezone)
    now = timezone.now()
    ...
    if evt.end:
        if evt.start.year != now.year or evt.end.year != evt.start.year:
            ...
            # Year can be skipped
            return format_interval(evt.start.date(), evt.end.date(), "MMMd", tzinfo=tz, locale=lo)
